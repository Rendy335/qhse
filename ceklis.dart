...
class Ceklis extends StatefulWidget {
  final int toilet;
  final String nama;
  const Ceklis({Key? key, required this.toilet, required this.nama})
      : super(key: key);

  @override
  // ignore: library_private_types_in_public_api, no_logic_in_create_state
  _CeklisState createState() => _CeklisState(this.toilet);
}

class _CeklisState extends State<Ceklis> {
  int? toilet;
  _CeklisState(this.toilet);
  List? data;
  ...
  void getData() async {
    ...
    var res = await http.post(urlAPI + phpceklis, body: {
      'access': 'apickb',
                        ...
                        children: <Widget>[
                          CheckboxListTile(
                            ...
                            onChanged: (bool? selected) {
                              _onCategorySelected(selected!, data?[i]["id"]);
                            },
